...
# (docker-compose.agent.yml). MinIO serves https so the pod can pull workspaces over TLS; the cert
# is self-signed, so the pod runs with AGENT_POD_WORKSPACE_TLS_VERIFY=0 and the runner's MinIO
# client with MINIO_CERT_CHECK=false. NOT for production.
#
