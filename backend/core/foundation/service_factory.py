...
class ServiceFactory:
    ...
    def create_service_from_config(self, config: Dict[str, Any]) -> BaseService:
        ...
        if "initializers" in config:
            for init_name in config["initializers"]:
                ...
                if not initializer:
                    raise ConfigurationError(
                        f"Initializer '{init_name}' not found for service '{name}'"
                    )
    ...
    async def health_check_all(self) -> Dict[str, Any]:
        ...
        # Combine results
        overall_healthy = (
            initializer_health.get("overall_status") == "healthy"
            ...
            "initializers": [
                {
                    ...
                    "config": {"base_url": "http://localhost:8080/v1/", "timeout": 30},
                    "dependencies": [],
                },
                {
                    "name": "database_initializer",
                    "class": "core.initializers.DatabaseInitializer",
                    "config": {"connection_string": "sqlite:///./database/app.db"},
                    "dependencies": [],
                },
            ],
            "services": [
                {
                    ...
                    "config": {"max_file_size_mb": 50, "supported_formats": ["pdf"]},
                    "dependencies": [],
                ...
                },
                {
                    ...
                    "config": {"analysis_timeout": 300, "confidence_threshold": 0.8},
                    ...
                    "stages": [
                        {
                            ...
                            "config": {"extract_structured_data": True},
                            "dependencies": [],
                            "parallel": False,
                            "retry": {"max_retries": 2, "timeout_seconds": 60},
                        },
                        {
                            ...
                            "parallel": False,
                            "retry": {"max_retries": 3, "timeout_seconds": 180},
                        },
                    ],
                }
            ],
