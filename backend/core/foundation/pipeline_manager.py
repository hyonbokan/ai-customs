...
class PipelineManager:
    ...
    async def execute_stage(
        self, pipeline_name: str, stage_name: str, input_data: Optional[Dict[str, Any]] = None
    ) -> Any:
        """
        ...
        if not stage:
            raise ConfigurationError(
                f"Stage '{stage_name}' not found in pipeline '{pipeline_name}'"
            )
        ...
        if not service:
            raise ConfigurationError(
                f"Service '{stage.service_name}' not found for stage '{stage_name}'"
            )
        ...
        if not service.is_running:
            ...
            if not success:
                raise ConfigurationError(
                    f"Failed to start service '{stage.service_name}' for stage '{stage_name}'"
                )
        ...
        for attempt in range(stage.max_retries + 1):
            try:
                logger.info(
                    f"Executing stage '{stage_name}' (attempt {attempt + 1}/{stage.max_retries + 1})"
                )
        ...
        raise ConfigurationError(
            f"Stage '{stage_name}' failed after {stage.max_retries + 1} attempts"
        )

    async def _execute_stage_with_service(
        self, service: BaseService, context: Dict[str, Any]
    ) -> Any:
        """
    ...
    async def execute_pipeline(
        self, pipeline_name: str, input_data: Optional[Dict[str, Any]] = None
    ) -> PipelineResult:
        """
        ...
        try:
            ...
            for batch_idx, batch in enumerate(stage_batches):
                logger.info(
                    f"Executing pipeline batch {batch_idx + 1}/{len(stage_batches)}: {batch}"
                )
                ...
                if len(batch) == 1:
                    ...
                    try:
                        stage_result = await self.execute_stage(
                            pipeline_name, stage_name, current_data
                        )
            ...
            await self._emit_event(
                "pipeline_completed", {"pipeline_name": pipeline_name, "result": result}
            )

            logger.info(
                f"Pipeline '{pipeline_name}' completed successfully in {result.duration_seconds:.2f}s"
            )
            return result
            ...
            await self._emit_event(
                "pipeline_failed", {"pipeline_name": pipeline_name, "error": str(e)}
            )
