...
class ServiceRegistry:
    ...
    def create_service(self, name: str, config: Optional[Dict[str, Any]] = None, 
                      dependencies: Optional[List[str]] = None) -> BaseService:
        """
    ...
    def _resolve_dependencies(self, services_to_start: Optional[List[str]] = None) -> List[List[str]]:
        """
    ...
    async def start_services(self, service_names: Optional[List[str]] = None, force: bool = False) -> bool:
        """
        ...
        try:
            ...
            for batch_idx, batch in enumerate(batches):
                ...
                if failed_services:
                    raise ConfigurationError(
                        f"Failed to start services: {failed_services}"
                    )
    ...
    async def stop_services(self, service_names: Optional[List[str]] = None, force: bool = False) -> bool:
        """
    ...
    async def get_metrics_all(self) -> Dict[str, Any]:
        ...
        for (name, task), result in zip(tasks, results):
            if isinstance(result, Exception):
                service_metrics[name] = {
                    "error": f"Metrics collection failed: {result}"
                }
            else:
        ...
        overall_metrics = {
            ...
            "healthy_services": sum(
                1 for metrics in service_metrics.values()
