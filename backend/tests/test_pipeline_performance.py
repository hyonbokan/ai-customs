        ...
        if response.status_code == 200:
            ...
            if complete_result:
                result["pipeline_metrics"]["complete_result_metrics"] = {
                    ...
                    "pdf_extraction_success": complete_result.get("pdf_extraction", {}).get("success", False),
                    "llm_analysis_success": complete_result.get("llm_analysis", {}).get("success", False),
                ...
                if pdf_extraction:
                    result["pipeline_metrics"]["pdf_extraction_details"] = {
                        ...
                        "ready_for_llm": pdf_extraction.get("metadata", {}).get("ready_for_llm", False)
                    }
        else:
            result["error"] = response.text
...
def save_parser_results_details(test_results: Dict[str, Any]):
    ...
    if full_pipeline.get("success") and full_pipeline.get("response_data"):
        ...
        if complete_result:
            complete_file = Path(PERFORMANCE_RESULTS_DIR) / f"complete_pipeline_result_{timestamp}.json"
...
def print_detailed_summary(test_results: Dict[str, Any]):
    ...
    # Health Check
    print(f"🔍 Health Check: {'✅ PASS' if health_check['success'] else '❌ FAIL'} "
          f"({health_check.get('response_time_ms', 'N/A')}ms)")
    
    # PDF Parsing Details
    print(f"📄 PDF Parsing: {'✅ PASS' if pdf_parsing['success'] else '❌ FAIL'} "
    ...
    print(f"🔄 Full Pipeline: {'✅ PASS' if full_pipeline['success'] else '❌ FAIL'} "
    ...
    if full_pipeline.get("success") and full_pipeline.get("pipeline_metrics"):
        ...
        if metrics.get("complete_result_metrics"):
            ...
            print(f"      • PDF Extraction Success: {cr_metrics.get('pdf_extraction_success', False)}")
            print(f"      • LLM Analysis Success: {cr_metrics.get('llm_analysis_success', False)}")
            print(f"      • Final Report Available: {cr_metrics.get('final_report_available', False)}")
        ...
        if metrics.get("pdf_extraction_details"):
            ...
            print(f"      • Text Content Length: {pdf_details.get('text_content_length', 0):,} characters")
